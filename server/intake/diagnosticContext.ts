 ...
 */
import { mediaReferenceCatalog, retrieveDiagnosticKnowledge } from './knowledge/index.js';
...
export function diagnosticContext(input: AnalyzeInput) {
  ...
  const records = matched.length ? matched : input.media.length ? mediaReferenceCatalog() : [];
  return {
