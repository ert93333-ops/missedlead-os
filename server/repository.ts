...
export class SupabaseRepository implements Repository {
  ...
  async expandStaleQuoteRequests(now:string) { const cutoff=new Date(new Date(now).getTime()-86_400_000).toISOString();const requests=await this.service.from("service_requests").select("id").in("workflow_status",["matched","quoted"]).lt("created_at",cutoff);if(requests.error)throw requests.error;for(const request of requests.data){const [matches,quotes,providers]=await Promise.all([this.service.from("request_matches").select("provider_id,rank").eq("request_id",request.id),this.service.from("quotes").select("id").eq("request_id",request.id),this.service.from("profiles").select("id").eq("role","provider").eq("provider_status","approved").eq("license_verified",true).gt("license_expires_at",now).eq("insurance_verified",true).gt("insurance_expires_at",now).contains("service_categories",["general"]).contains("service_areas",["Charlotte"])]);if(matches.error||quotes.error||providers.error)throw matches.error??quotes.error??providers.error;if(quotes.data.length>=3)continue;const existing=new Set(matches.data.map(v=>v.provider_id));const selected=providers.data.filter(v=>!existing.has(v.id)).sort((a,b)=>Number(explorationSelected(request.id,b.id))-Number(explorationSelected(request.id,a.id))||a.id.localeCompare(b.id)).slice(0,Math.max(0,3-matches.data.length));if(selected.length){const inserted=await this.service.from("request_matches").insert(selected.map((v,index)=>({request_id:request.id,provider_id:v.id,rank:matches.data.length+index+1,exploration_selected:explorationSelected(request.id,v.id)})));if(inserted.error)throw inserted.error;}const updated=await this.service.from("service_requests").update({expanded_search:matches.data.length+selected.length<3}).eq("id",request.id);if(updated.error)throw updated.error;} }
  async dashboard(actor: Actor, token: string) {
    ...
    return {
      ...
      quotes:quotes.map(v=>{const snapshot=rankByQuote.get(String(v.id));return{id:String(v.id),requestId:String(v.request_id),providerId:String(v.provider_id),providerName:String(v.provider_name??"Provider"),scope:String(v.scope),amountCents:Number(v.amount_cents),ranking:v.ranking as Quote["ranking"],rankingScore:Number(snapshot?.score??0),rankingPolicyVersion:Number(snapshot?.policy_version??0),explorationSelected:Boolean(snapshot?.exploration_selected),createdAt:String(v.created_at)}}).sort((a,b)=>compositeQuoteScore(b)-compositeQuoteScore(a)||a.id.localeCompare(b.id)),
