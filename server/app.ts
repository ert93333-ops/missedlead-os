...
export function createApp(options: AppOptions = {}) {
  ...
  app.get("/api/dashboard", async (req, res) => { const data = await repository.dashboard(actor(res), req.header("Authorization")?.slice(7) ?? ""); res.json(data); });
