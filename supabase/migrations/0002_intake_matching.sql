...
alter table service_requests
  add column intake_assessment_id text,
  add constraint service_requests_intake_assessment_id_length
    check (intake_assessment_id is null or length(intake_assessment_id) between 1 and 200);

create unique index service_requests_customer_assessment_unique
  on service_requests(customer_id,intake_assessment_id)
  where intake_assessment_id is not null;

create function confirm_intake(
  p_customer_id uuid,
...
begin
  ...
  if not exists(
    select 1 from profiles p where p.id=p_customer_id and p.role='customer'
  ) then
  ...
  if v_request.id is null then
    ...
    if v_request.id is null then
      ...
      select v_request.id,candidate.id,candidate.rank,
      ...
      from (
        ...
        where p.role='provider'
          ...
          and p.license_expires_at>statement_timestamp()
          and p.insurance_verified
          and p.insurance_expires_at>statement_timestamp()
          and p_category=any(p.service_categories)
          and 'Charlotte'=any(p.service_areas)
        order by p.id
        limit 3
      ) candidate;

      select count(*)::integer into v_match_count
      from request_matches rm where rm.request_id=v_request.id;
  ...
  return jsonb_build_object(
    ...
    'status',v_request.workflow_status,
