...
test("operator resolves evidence-backed dispute, refunds, and sees immutable audit events", async ({ page }) => {
  ...
  await expect(page.getByRole("status")).toContainText("해결");
  await expect(page.getByTestId("audit-log")).toContainText("dispute.resolved");
  await page.getByLabel("환불 금액 USD").fill("25");
  await page.getByLabel("환불 사유").fill("Evidence-supported remediation");
  await page.getByRole("button", { name: "환불 실행" }).click();
  await expect(page.getByRole("status")).toContainText("환불");
  await expect(page.getByTestId("audit-log")).toContainText("refund.completed");
  await page.getByLabel("공급자 ID").fill("provider-new");
  await page.getByLabel("조직명").fill("New Provider");
  await page.getByLabel("면허 만료").fill("2027-09-01T09:00");
  await page.getByLabel("보험 만료").fill("2027-09-01T09:00");
  await page.getByRole("button",{name:"자격 저장"}).click();
  await expect(page.getByRole("status")).toContainText("자격");
  await page.getByRole("button",{name:"복구 목록 새로고침"}).click();
  await expect(page.getByTestId("recovery-console")).toContainText("claim-stuck");
  await page.getByRole("button",{name:"미수금 해결"}).click();
