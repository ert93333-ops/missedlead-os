  ...
  return <section className="chat-intake" data-testid="customer-intake" aria-labelledby="intake-title">
    ...
    <div className="chat-log" ref={logRef} role="log" aria-live="polite" aria-relevant="additions">
      ...
      {assessment && !busy && <div className="assessment" data-testid="intake-assessment">
        {assessment.safety.level !== "normal" && <div className={`safety-guidance safety-guidance--${assessment.safety.level}`} role="alert"><strong>{assessment.safety.level === "emergency" ? (locale === "es" ? "Posible emergencia" : "Possible emergency") : (locale === "es" ? "Atención urgente" : "Urgent attention")}</strong><p>{assessment.safety.guidance}</p></div>}
        {assessment.issueCandidates.length > 0 && <section className="possible-issues" aria-labelledby="possible-issues-title"><h2 id="possible-issues-title">{text.possibleIssues}</h2><p className="tentative-note">{text.tentative}</p>{assessment.issueCandidates.map((candidate) => <article key={candidate.id}><div><strong>{candidate.label}</strong><span className={`likelihood likelihood--${candidate.likelihood}`}>{candidate.likelihood}</span></div><p>{candidate.reason}</p>{candidate.evidenceNeeded.length > 0 && <small><PhotoIcon size={16}/>{text.evidence}: {candidate.evidenceNeeded.join(", ")}</small>}</article>)}</section>}
