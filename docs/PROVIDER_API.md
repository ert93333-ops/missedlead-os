...
- `GET /api/requests/:id/permit` request participants → `{permit:object|null}` with `permit_number,inspection_status,verified,verification_reference,verified_by,updated_at`.
- `PUT /api/requests/:id/permit` provider (eligible for the request) or operator `{permitNumber,inspectionStatus:'pending'|'passed'|'failed',verificationReference?}` → `{permit}`. Operator submissions set `verified=true` and record the reference; provider submissions reset `verified=false`. Permit-required jobs cannot start or complete until verified.
- `GET /api/ops/permits` operator only → `{permits:object[]}` joined with `service_requests(description,address)`, newest first.
